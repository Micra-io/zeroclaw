...
pub struct WhatsAppWebChannel {
    ...
    /// wa-rs 0.5: `Bot::run()` now returns `BotHandle` (a Future + abort)
    /// rather than a tokio JoinHandle directly (oxidezap/whatsapp-rust BotHandle wrapper).
    bot_handle: Arc<Mutex<Option<wa_rs::bot::BotHandle>>>,
...
impl WhatsAppWebChannel {
    ...
    async fn synthesize_voice_static(
        ...
        use wa_rs::upload::UploadOptions;
        ...
        // wa-rs 0.5: UploadResponse cryptographic fields became `[u8; 32]`
        // for type safety. Pull the Vec<u8> copies before consuming the
    ...
    async fn send_wa_attachment(
        ...
        // wa-rs 0.5: UploadResponse exposes crypto fields as `[u8; 32]`. Pull
        // the Vec<u8> copies up front so the move of `upload.direct_path`
...
impl Channel for WhatsAppWebChannel {
    ...
    async fn send(&self, message: &SendMessage) -> Result<()> {
        ...
        // wa-rs 0.5: send_message returns `SendResult { message_id, to }`
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        *self.tx.lock() = Some(tx.clone());
        ...
        loop {
            ...
            // wa-rs 0.5: BotBuilder gained a 4th typestate slot for the async
            // runtime (oxidezap/whatsapp-rust#621). `with_runtime` is required
            // before `.build()` resolves; we use the bundled `TokioRuntime`.
            // `with_device_props` switched from three positional Options to a
            // `DevicePropsOverride` builder (oxidezap/whatsapp-rust#586).
            let mut builder = Bot::builder()
                ...
                .on_event(move |event, client| {
                    ...
                    async move {
                        // wa-rs 0.5: event handlers receive `Arc<Event>` per
                        // PR #613, so we match against `&*event` to get a
                        ...
                        match &*event {
                            Event::Message(msg, info) => {
                                ...
                                // wa-rs 0.5: `Client::get_phone_number_from_lid`
                                ...
                                // the phone field on hit, swallow lookup
                                // errors back to `None` (consistent with the
                                // legacy semantics — best-effort enrichment).
