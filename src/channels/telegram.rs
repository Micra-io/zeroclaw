...
impl TelegramChannel {
    pub fn new(bot_token: String, allowed_users: Vec<String>, mention_only: bool, allowed_chats: Vec<String>) -> Self {
...
Allowlist Telegram username (without '@') or numeric user ID.",
    ...
    async fn try_parse_attachment_message(
        ...
        if !Self::is_chat_allowed(&chat_id, chat_type, &self.allowed_chats) {
            tracing::debug!(
                "Telegram: ignoring attachment in chat {chat_id} (type={chat_type}): not in allowed_chats"
            );
            return None;
        }
...
mod tests {
    ...
    fn telegram_channel_name() {
        let ch = TelegramChannel::new("fake-token".into(), vec!["*".into()], false, vec![]);
    ...
    fn supports_draft_updates_respects_stream_mode() {
        let off = TelegramChannel::new("fake-token".into(), vec!["*".into()], false, vec![]);
        assert!(!off.supports_draft_updates());

        let partial = TelegramChannel::new("fake-token".into(), vec!["*".into()], false, vec![])
    ...
    #[test]
    fn telegram_api_url() {
        let ch = TelegramChannel::new("123:ABC".into(), vec![], false, vec![]);
        assert_eq!(
            ch.api_url("getMe"),
    ...
    fn telegram_user_allowed_wildcard() {
        let ch = TelegramChannel::new("t".into(), vec!["*".into()], false, vec![]);
        assert!(ch.is_user_allowed("anyone"));
    }

    #[test]
    fn telegram_user_allowed_specific() {
        let ch = TelegramChannel::new("t".into(), vec!["alice".into(), "bob".into()], false, vec![]);
    ...
    fn telegram_user_allowed_with_at_prefix_in_config() {
        let ch = TelegramChannel::new("t".into(), vec!["@alice".into()], false, vec![]);
        assert!(ch.is_user_allowed("alice"));
    }

    #[test]
    fn telegram_user_denied_empty() {
        let ch = TelegramChannel::new("t".into(), vec![], false, vec![]);
        assert!(!ch.is_user_allowed("anyone"));
    }
    ...
    fn telegram_wildcard_with_specific_users() {
        let ch = TelegramChannel::new("t".into(), vec!["alice".into(), "*".into()], false, vec![]);
        assert!(ch.is_user_allowed("alice"));
    ...
    fn telegram_user_allowed_by_numeric_id_identity() {
        let ch = TelegramChannel::new("t".into(), vec!["123456789".into()], false, vec![]);
        assert!(ch.is_any_user_allowed(["unknown", "123456789"]));
    }

    #[test]
    fn telegram_user_denied_when_none_of_identities_match() {
        let ch = TelegramChannel::new("t".into(), vec!["alice".into(), "987654321".into()], false, vec![]);
        assert!(!ch.is_any_user_allowed(["unknown", "123456789"]));
    }
    ...
    fn parse_update_message_uses_chat_id_as_reply_target() {
        let ch = TelegramChannel::new("token".into(), vec!["*".into()], false, vec![]);
        let update = serde_json::json!({
            "update_id": 1,
            "message": {
                "message_id": 33,
    ...
    fn parse_update_message_allows_numeric_id_without_username() {
        let ch = TelegramChannel::new("token".into(), vec!["555".into()], false, vec![]);
        let update = serde_json::json!({
            "update_id": 2,
            "message": {
                "message_id": 9,
    ...
    fn parse_update_message_mention_only_group_requires_exact_mention() {
        let ch = TelegramChannel::new("token".into(), vec!["*".into()], true, vec![]);
        {
    ...
    fn telegram_mention_only_enabled_by_config() {
        ...
        let ch_disabled = TelegramChannel::new("token".into(), vec!["*".into()], false, vec![]);
    ...
    fn with_transcription_sets_config_when_enabled() {
        ...
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false, vec![]).with_transcription(tc);
    ...
    async fn try_parse_voice_message_rejects_unauthorized_sender_before_download() {
        ...
        let ch = TelegramChannel::new("token".into(), vec!["alice".into()], false, vec![])
    ...
    async fn e2e_live_voice_transcription_and_reply_cache() {
        ...
        let ch = TelegramChannel::new("test_token".into(), vec!["*".into()], false, vec![]);
    ...
    fn with_ack_reactions_false_disables_reactions() {
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false, vec![]).with_ack_reactions(false);
        assert!(!ch.ack_reactions);
    }
    ...
    fn with_ack_reactions_true_keeps_reactions() {
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false, vec![]).with_ack_reactions(true);
        assert!(ch.ack_reactions);
    }
