...
fn load_group_context(
    ...
    // Format with timestamps so the LLM can reason about recency. `created_at`
    // is an `Option<DateTime<Utc>>` (upstream's `TimestampedMessage`); when it
    // is `None` (unparseable/absent) the timestamp prefix is omitted.
    let mut lines: Vec<String> = messages
        .iter()
        .map(|m| match m.created_at {
            Some(dt) => format!("[{}] {}", dt.format("%H:%M"), m.message.content),
            None => m.message.content.clone(),
        })
        .collect();
...
async fn process_channel_message_body(
    ...
    // Runs BEFORE the thinking `system_prompt_prefix` wrap (below) so the
    // injected group transcript becomes part of the prompt the prefix wraps.
    ...
    mod group_context {
        ...
        fn load_group_context_falls_back_to_last_n_when_window_empty() {
            ...
            // Insert messages older than the 15-minute window
            {
                let conn = store.conn().lock();
                for i in 0..5_i64 {
                    let ts = format!("2026-03-20T10:{i:02}:00+00:00");
                    conn.execute(
                        "INSERT INTO sessions (session_key, role, content, created_at) VALUES (?1, 'user', ?2, ?3)",
