...
impl Agent {
    ...
    async fn execute_tool_call(&self, call: &ParsedToolCall) -> ToolExecutionResult {
        ...
        // Serialize arguments once (after hooks may have mutated them) so the
        // same string is available for both observer events.
                ...
                match tool.execute(tool_args.clone()).await {
                    Ok(r) => {
                        ...
                        self.observer.record_event(&ObserverEvent::ToolCall {
                            ...
                            result: Some(crate::agent::loop_::scrub_credentials(&outcome_text)),
                        });
                        (outcome_text, ok)
                    }
                    Err(e) => {
                        ...
                        self.observer.record_event(&ObserverEvent::ToolCall {
                            ...
                            success: false,
                            arguments: Some(args_json.clone()),
                            result: Some(crate::agent::loop_::scrub_credentials(&err_text)),
                        });
                        (err_text, false)
                    }
                }
