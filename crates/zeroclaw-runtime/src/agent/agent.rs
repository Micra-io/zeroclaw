...
impl Agent {
    ...
    fn trim_history(&mut self) {
        ...
        let threshold = if self.config.resolved.history_trim_chunked {
    ...
    fn trim_history_does_not_leave_orphan_tool_results() {
        ...
        let agent_config = zeroclaw_config::schema::AliasedAgentConfig {
            resolved: zeroclaw_config::schema::ResolvedRuntime {
                ...
                // count and the orphan-guard / cascade paths are exercised.
                // Chunked mode (default) only trims at 2*max.
