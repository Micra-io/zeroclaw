...
impl SqliteMemory {
    ...
    async fn store_row_with_metadata(
        ...
        // Fork: structured metadata JSON blob (e.g. WhatsApp `{"group_jid":...}`)
        // persisted verbatim on the row.
    ...
    async fn recall_by_time_only(
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<Vec<MemoryEntry>> {
            ...
            let mut sql =
                "SELECT m.id, m.key, m.content, m.category, m.created_at, m.session_id, m.namespace, m.importance, m.superseded_by, m.kind, m.pinned, a.alias, m.agent_id, m.tenant_id, m.metadata \
...
impl Memory for SqliteMemory {
    ...
    async fn store_with_options(
        ...
        self.store_row_with_metadata(key, content, category, session_id, options, None, None)
            .await
    }
